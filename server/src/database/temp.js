import { defaultPool ,host_pool} from "../config/db.config.js";
import dotenv from "dotenv";
dotenv.config();

const db_name = process.env.DB_NAME || "educonnect";
const pool = host_pool

export const createDatabase = async () => {
  try {
    const checkQuery = `SELECT 1 FROM pg_database WHERE datname= $1`;
    const res = await host_pool.query(checkQuery, [db_name]);
    if (res.rowCount === 0) {
      await host_pool.query(`CREATE DATABASE ${db_name}`);
