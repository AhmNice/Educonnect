...
export const createTables = async () => {
  ...
  CHECK (max_members = 0 OR current_members <= max_members)
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS learning.course_resources (
        ...
        uploader_id UUID REFERENCES core.users(user_id),
  ...
  `);
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS analytics.reports (
        ...
        reporter_id UUID REFERENCES core.users(user_id),
        reported_user_id UUID REFERENCES core.users(user_id),
        reason TEXT,
  ...
  `)
    ...
    await pool.query(`
      ...
      ON core.users(university_id);
    `);

    // Study Groups indexes
    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_study_groups_course_id
      ON learning.study_groups(course_id);
    `);

    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_study_groups_created_by
      ON learning.study_groups(created_by);
    `);

    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_study_groups_created_at
      ON learning.study_groups(created_at DESC);
    `);

    // Fast tag filtering
    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_study_groups_tags_gin
      ON learning.study_groups USING GIN (tags);
    `);

    // Study Group Members index
    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_group_members_group_id
      ON learning.study_group_members(group_id);
    `);

    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_group_members_user_id
      ON learning.study_group_members(user_id);
    `);

    // Courses
    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_courses_department
      ON learning.courses(department);
    `);

    // Analytics performance
    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_user_activity_user_id
      ON analytics.user_activity(user_id);
    `);

    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_reports_reporter_id
      ON analytics.reports(reporter_id);
    `);

    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_reports_reported_user_id
      ON analytics.reports(reported_user_id);
    `);
